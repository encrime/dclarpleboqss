(function () {
  ...
  function shouldHideNativeBadge(b) {
    ...
    if (h.quest) {
      if (id.indexOf("quest") !== -1 || desc.indexOf("quest") !== -1) {
        return true;
      }
  ...
  /*
   * FIX:
   * Parsing is now strict and always returns either a valid timestamp
   * or null. Invalid dates are never allowed to propagate.
   */
  function parseAccountDateIsoMs(s) {
    ...
    /*
     * Make sure Date.UTC did not normalize an invalid date,
     * e.g. 31/02/2020.
     */
  ...
  function getUsernameSpoofReplace(user) {
    ...
    if (
      normName(storage.matchUsername || "") === pun &&
      (storage.replaceUsername || "").trim()
    ) {
  ...
  function shouldWrapUserForProxy(user) {
    ...
    if (getUsernameSpoofReplace(user)) {
      return true;
    }
    ...
    var proxy = new Proxy(user, {
      get: function (t, p, recv) {
        ...
        if (
          ...
          /*
           * FIX:
           * Return a serializable ISO string instead of
        ...
        if (
          ...
          if (
            ...
            return {
              configurable: true,
              enumerable: true,
              value: spoofCreatedMs
            };
          }

          /*
           * FIX:
           * Keep the descriptor consistent with get().
           */
          if (
            p === "createdAt" ||
            p === "created_at"
   ...
   * FIX IMPORTANT:
   *
   * The previous version patched internal Snowflake
   * converter functions by scanning Discord's modules.
   *
   * This is removed because returning an account-date
   * timestamp from an arbitrary internal converter can
   * break Discord's assumptions and crash the app.
   */
  ...
  function patchBadges() {
    try {
      ...
      function badgeHandler(
        ...
        function applyNonSpoofLocalFilters(
          arr
        ) {
          return arr.filter(
            function (x) {
